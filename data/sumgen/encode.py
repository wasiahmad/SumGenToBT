...
class MultiprocessingEncoder(object):
    ...
    def encode(self, input):
        ...
        if len(target) == 0 and not self.args.keep_empty:
            return ["EMPTY", None, None]
        src_tokens = self._encode(source)
        tgt_tokens = self._encode(target)
        if self.args.strict_max_len:
            if len(src_tokens) > self.args.max_len or \
                    len(tgt_tokens) > self.args.max_len:
                return ["EMPTY", None, None]
        else:
            src_tokens = src_tokens[:self.args.max_len]
            tgt_tokens = tgt_tokens[:self.args.max_len]
...
def main():
    ...
    parser.add_argument("--strict_max_len", action="store_true", help="remove examples exceeding max_len")
